...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        openHydrationShortcut.setOnClickListener(view -> addHomeHydration());
        ...
        findViewById(R.id.chipStartWalk).setOnClickListener(view -> logHomeWalk());
    ...
    private void updateHomeStats() {
        ...
        int calorieGoal = Math.max(100, preferences.getInt(FitnessState.KEY_CALORIE_GOAL, FitnessState.DEFAULT_CALORIE_GOAL));
        ...
        int progress = FitnessState.goalProgress(activeCalories, calorieGoal);
        float distanceKm = FitnessState.distanceKm(steps);
        int readiness = calculateReadiness(progress, hydrationMl, steps);
        int streakDays = preferences.getInt(FitnessState.KEY_STREAK_DAYS, 0);
        String lastWorkout = preferences.getString(FitnessState.KEY_LAST_WORKOUT, getString(R.string.none_label));
        float lastBmi = preferences.getFloat(FitnessState.KEY_LAST_BMI, 0f);
        String lastBmiCategory = preferences.getString(FitnessState.KEY_LAST_BMI_CATEGORY, "");
        ...
        if (lastBmi > 0f && lastBmiCategory != null && !lastBmiCategory.isEmpty()) {
            textViewResult.setText(getString(R.string.bmi_result_short, lastBmi, lastBmiCategory));
        }
    ...
    private void addHomeHydration() {
        FitnessState.addHydration(this, 250);
        Toast.makeText(this, getString(R.string.hydration_added), Toast.LENGTH_SHORT).show();
        updateHomeStats();
    }

    private void logHomeWalk() {
        int addedSteps = FitnessState.logWorkout(this, getString(R.string.walk_mode), 30);
        Toast.makeText(this, getString(R.string.home_walk_logged, addedSteps), Toast.LENGTH_SHORT).show();
        updateHomeStats();
    }

    private void startTrackingIfPermissionReady() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q
                && ContextCompat.checkSelfPermission(this, Manifest.permission.ACTIVITY_RECOGNITION)
                != PackageManager.PERMISSION_GRANTED) {
            return;
        }
        ContextCompat.startForegroundService(this, new Intent(this, StepTrackingService.class));
    }
